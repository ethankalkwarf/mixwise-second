...
interface CocktailFunFactCardProps {
  fact: string;
  sources?: Array<{ label: string; url: string }>;
}

export function CocktailFunFactCard({ fact, sources }: CocktailFunFactCardProps) {
  return (
    <div className="bg-amber-50 rounded-2xl p-6 md:p-8 border border-amber-100 relative overflow-hidden">
      {/* Decorative Background Icon */}
      <LightBulbIcon className="absolute -top-4 -right-4 w-24 h-24 text-amber-100 -rotate-12" />
