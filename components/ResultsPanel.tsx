...
export function ResultsPanel({
  ...
  const stapleIds = useMemo(() => {
    return allIngredients.filter((i) => i.is_staple).map((i) => i.id);
  }, [allIngredients]);
  ...
  const unlockPotential = useMemo(() => {
    ...
    almostThere.forEach((match) => {
      ...
      if (!missingId) return;
      const current = immediateUnlockCounts.get(missingId) || {
        count: 0,
        drinks: []
      };
      current.count += 1;
    ...
    allCocktails.forEach((c) => {
      c.ingredients.forEach((ing) => {
        totalUsageCounts.set(
          ing.id,
          (totalUsageCounts.get(ing.id) || 0) + 1
        );
      });
    });
  ...
  const missingForSelected = useMemo(() => {
    ...
    return selectedCocktail.ingredients
      .filter(
        (i) =>
          !i.isOptional && !invSet.has(i.id) && !stapleSet.has(i.id)
      )
      .map((i) => i.id);
