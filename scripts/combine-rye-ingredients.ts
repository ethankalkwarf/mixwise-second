...
/**
 * Combine Rye and Rye Whiskey into a single catalog ingredient.
 * Keep "Rye Whiskey" as canonical (guide slug rye-whiskey, aliases include "rye").
 *
 * Usage:
 *   npx tsx scripts/combine-rye-ingredients.ts           # dry run
 *   npx tsx scripts/combine-rye-ingredients.ts --confirm # apply
 */
...
const CANONICAL_NAME = "Rye Whiskey";
const DUPLICATE_NAME = "Rye";
  ...
  if (table === "cocktail_ingredients") {
    ...
    for (const row of dupRows || []) {
      const { data: existing } = await supabase
        .from(table)
        .select("cocktail_id")
        .eq("cocktail_id", row.cocktail_id)
        .eq("ingredient_id", toId)
        .maybeSingle();
      ...
      if (existing) {
        const { error } = await supabase
          ...
          .eq("ingredient_id", fromId);
        if (error) throw error;
        console.log(`    deleted duplicate cocktail_ingredients for cocktail ${row.cocktail_id}`);
      } else {
        const { error } = await supabase
          .from(table)
          .update({ ingredient_id: Number(toId) })
          .eq("cocktail_id", row.cocktail_id)
          .eq("ingredient_id", fromId);
        if (error) throw error;
        console.log(`    remapped cocktail_ingredients for cocktail ${row.cocktail_id}`);
      }
    }
    return;
  }
...
async function combineRyeIngredients(dryRun: boolean) {
  console.log("COMBINING RYE INGREDIENTS");
  console.log("=========================\n");
  ...
  if (!ingredients?.length) {
    console.log("No rye ingredients found.");
    return;
  }
  ...
  if (!dryRun) {
    for (const table of tables) {
      await remapDuplicate(table, String(duplicate.id), String(canonical.id), dryRun);
    }
  ...
  console.log("\nRemaining rye rows:", remaining);
  console.log(dryRun ? "\nDry run complete." : "\nMerge complete.");
}

const dryRun = !process.argv.includes("--confirm");
combineRyeIngredients(dryRun).catch((error) => {
  console.error(error);
  process.exit(1);
});
